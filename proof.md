...
\[
S\ge H_{\rm cert}N+\operatorname{tr}\Psi(M)-o(N),
...
\qquad \Delta=\operatorname{tr}\Psi(G).
\]
...
\(t=\operatorname{tr}G\le m\).  Cauchy--Schwarz gives
...
\operatorname{tr}\Psi(G_B)
 +\eta B\operatorname{span}(B)\ge R-o(1).
\]
...
\[
\frac SN\ge
\frac{mH_{\rm cert}-\eta B(m-1)}{m-R}.
\]
